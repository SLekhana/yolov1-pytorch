...
class YOLOv1Module(pl.LightningModule):
    def __init__(
        ...
        _model = YOLOv1(S, B, C)
        self.model = torch.compile(_model) if compile_model else _model
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        return DataLoader(
            ds, batch_size=self.hparams.batch_size, shuffle=True, num_workers=num_workers, pin_memory=True, **kwargs
        )

    def val_dataloader(self) -> DataLoader:
        ds = VOCDataset(self.hparams.data_root, year="2007", split="test")
        return DataLoader(ds, batch_size=self.hparams.batch_size, num_workers=self.hparams.num_workers, pin_memory=True)
