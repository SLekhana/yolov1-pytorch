...
class YOLOv1Loss(nn.Module):
    def __init__(
        self, S: int = 7, B: int = 2, C: int = 20, lambda_coord: float = 5.0, lambda_noobj: float = 0.5
    ) -> None:
        super().__init__()
        self.S = S
        self.B = B
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        coord_loss = self.lambda_coord * (obj * (pred_boxes[..., 1:3] - tgt_box.unsqueeze(-2)[..., :2]).pow(2)).sum()
        coord_loss += (
            self.lambda_coord
            * (
                obj
                * (pred_boxes[..., 3:5].clamp(min=0).sqrt() - tgt_box.unsqueeze(-2)[..., 2:].clamp(min=0).sqrt()).pow(2)
            ).sum()
        )
